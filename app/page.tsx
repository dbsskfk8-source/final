  ...
  return (
    <div className="min-h-screen bg-[#fffdfa] text-[#333]">
      ...
      <main className="pt-32 pb-20">
        ...
        <section className="px-10 lg:px-20 grid md:grid-cols-2 gap-20 items-center mb-32">
          <div className="max-w-xl">
            <h1 className="text-[3.5rem] md:text-[5rem] font-extrabold leading-[1.05] tracking-tight mb-8">
              The art of <span className="text-[#566e63] italic font-serif">letting go.</span>
            </h1>
            <p className="text-lg md:text-xl text-gray-500 font-medium mb-12 leading-relaxed">
              A sanctuary for emotional transition. We provide the tools and space to release the weights you've carried, turning endings into peaceful beginnings.
            </p>
        ...
        <section className="px-10 lg:px-20">
          <div className="flex justify-between items-end mb-16">
            <div>
              <h2 className="text-4xl font-extrabold mb-4">Space to Breathe</h2>
              <p className="text-gray-400 font-medium">Explore our curated pathways for emotional release and personal restoration.</p>
            </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            {contents && contents.find(c => c.title === 'Deep Emotional Cleansing') && (
              <div className="md:col-span-2 bg-[#e8efe9] rounded-[40px] p-12 relative flex flex-col justify-end min-h-[500px] group overflow-hidden">
                ...
                <div className="relative">
                  <span className="bg-white/40 text-[#566e63] px-4 py-1 rounded-full text-[10px] font-bold tracking-widest uppercase mb-6 inline-block">Therapeutic Presence</span>
                  <h3 className="text-4xl font-extrabold mb-4">{contents.find(c => c.title === 'Deep Emotional Cleansing')?.title}</h3>
                  ...
                  <Link href="/chat" className="flex items-center gap-2 font-bold text-[#566e63] group-hover:gap-4 transition-all">
                    Explore Session <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                  </Link>
                </div>
              </div>
            )}
            ...
            <div className="flex flex-col gap-8">
              {contents && contents.filter(c => c.category === 'service').map(service => (
                <div key={service.id} className="bg-gray-50/50 rounded-[40px] p-8 flex-1 group hover:bg-white hover:shadow-xl transition-all border border-transparent hover:border-gray-50">
                  <div className="w-12 h-12 bg-white rounded-2xl shadow-sm flex items-center justify-center text-[#566e63] mb-12">
                     <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                        <path d={service.title === 'Guided Care' ? "M12 2C8 2 4 5 4 10C4 16.5 12 22 12 22S20 16.5 20 10C20 5 16 2 12 2Z" : "M21 21H3V3h18v18zM12 6c-3.3 0-6 2.7-6 6s2.7 6 6 6 6-2.7 6-6-2.7-6-6-6z"}/>
                     </svg>
                  </div>
                  <h4 className="text-xl font-extrabold mb-3">{service.title}</h4>
                  <p className="text-gray-400 text-sm font-medium line-clamp-3">{service.description}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="mt-32 border-t border-gray-100 pt-16 px-10 flex flex-col items-center">
            ...
            <div className="flex gap-20 text-xs font-bold text-gray-400 uppercase tracking-widest">
      ...
      <footer className="px-10 py-10 border-t border-gray-50 flex justify-between items-center bg-[#fafafa]/50">
        <div>
          <div className="font-extrabold text-sm mb-1">Final Service</div>
