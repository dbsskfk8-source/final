    ...
    <div className="min-h-screen bg-[#fcfdfc] font-sans text-[#333] pb-24">
      <header className="px-4 md:px-10 py-4 md:py-6 flex justify-between items-center max-w-[1400px] mx-auto border-b border-gray-100 bg-white/80 backdrop-blur-md sticky top-0 z-50">
        <Link href="/" className="font-extrabold text-2xl md:text-3xl tracking-tight text-[#4a5c53] flex items-center gap-2 shrink-0">
          <BrainCircuit size={24} className="md:size-[28px]" /> MoodB
        </Link>
      ...
      <main className="max-w-[1200px] mx-auto px-6 pt-10">
        ...
        <div className="mb-12 animate-in fade-in slide-in-from-bottom-6 duration-500 delay-100">
          <div className="grid grid-cols-7 gap-1.5 md:gap-3">
            {scores.map((score, idx) => (
              <div key={idx} className={`flex flex-col items-center justify-center p-1.5 sm:p-3 rounded-xl sm:rounded-2xl border ${GROUP_COLOR[score.group]} transition-all`} >
                <span className="text-base font-bold tracking-widest text-current/60 mb-1 sm:mb-2 truncate max-w-full">
                  {score.subject.replace(/[^가-힣]/g, '')}
                </span>
            ...
            <div className="w-full max-w-lg h-[350px] md:h-[400px]">
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart cx="50%" cy="50%" outerRadius="70%" data={chartData}>
                  ...
                  <Radar name="정상 범위" dataKey="max" stroke="#566e63" strokeWidth={1} strokeDasharray="4 4" fill="#566e63" fillOpacity={0.15} isAnimationActive={false} />
        ...
        <div className="grid md:grid-cols-5 gap-8 mb-16 animate-in fade-in slide-in-from-bottom-8 duration-500 delay-200">
            ...
            <div className="w-full h-[280px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={scores} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <XAxis dataKey="subject" tick={{ fontSize: 14, fill: '#666', fontWeight: 'bold' }} axisLine={false} tickLine={false} />
                  <YAxis domain={[0, 100]} tick={{ fontSize: 14, fill: '#999' }} axisLine={false} tickLine={false} />
                  <RechartsTooltip contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 30px rgba(0,0,0,0.08)' }} formatter={(val: any, name: any, props: any) => [ val, props.payload.groupLabel ]} />
                  <Line type="monotone" dataKey="A" stroke="#566e63" strokeWidth={3} dot={{ r: 5, fill: '#566e63', strokeWidth: 2, stroke: '#fff' }} activeDot={{ r: 8 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="md:col-span-2 bg-[#f9faf9] p-8 md:p-10 rounded-[32px] flex flex-col justify-between">
            ...
            <Link href={`/report`} className="mt-8 bg-white border border-gray-200 text-[#4a5c53] font-bold py-3 px-8 rounded-full inline-flex items-center gap-2 hover:bg-gray-50 transition-colors shadow-sm self-start group">
              <BrainCircuit size={16} className="group-hover:text-[#566e63]" /> 의학적 심층 리포트 보기 <ArrowRight size={16} className="text-gray-400 group-hover:translate-x-1 transition-transform" />
            </Link>
          </div>
        </div>

        {/* ✨ 명상 추천 카드 섹션 (7a1c559 복원) */}
        <div className="mb-20 animate-in fade-in slide-in-from-bottom-10 duration-500 delay-300">
          <div className="text-center max-2-2xl mx-auto mb-10">
          ...
          {attentionRequired.length > 0 ? (
            <div className="flex flex-wrap justify-center gap-6">
              {attentionRequired.map((score, idx) => (
                <Link href={`/meditation/${encodeURIComponent(score.subject)}`} key={idx} className="w-full sm:w-[280px] bg-white border-2 border-[#eaeced] hover:border-[#566e63] rounded-[24px] p-8 text-center group hover:shadow-xl hover:-translate-y-1 transition-all flex flex-col items-center" >
        ...
        {allResults.length > 1 && (
          <div className="mb-20 pt-10 border-t border-gray-200 animate-in fade-in slide-in-from-bottom-8 duration-500 delay-500">
            <h3 className="text-xl font-extrabold text-[#222] mb-6 flex items-center gap-2">
              <Calendar size={20} className="text-[#566e63]" /> 나의 이전 진단 목록
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {allResults.map((rec, idx) => {
                ...
                return (
                  <Link key={idx} href={isCurrent ? '#' : `/result?id=${recId}`} className={` flex flex-col p-4 rounded-2xl border transition-all text-left ${isCurrent ? 'bg-[#566e63] border-[#566e63] text-white shadow-md' : 'bg-white border-gray-200 hover:border-[#566e63] hover:shadow-sm group text-[#333] cursor-pointer' } `} >
