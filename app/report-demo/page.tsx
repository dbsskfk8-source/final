...
interface ResultData {
  ...
  overall_t_score?: number
}

function ReportDemoContent() {
          ...
          resultsArray = [
            {
              ...
              overallTScore: 68.5,
              scores: [
                { name: '기쁨', subject: '기쁨(喜)', factor: 'JOY', tScore: 52, A: 52, group: 'normal', groupLabel: '정상 범위' },
                { name: '분노', subject: '분노(怒)', factor: 'ANGER', tScore: 78, A: 78, group: 'risk', groupLabel: '위험 범위' },
                { name: '생각', subject: '생각(思)', factor: 'THOUGHT', tScore: 65, A: 65, group: 'caution', groupLabel: '주의 단계' },
                { name: '우울', subject: '우울(憂)', factor: 'DEPRESSION', tScore: 55, A: 55, group: 'normal', groupLabel: '정상 범위' },
                { name: '슬픔', subject: '슬픔(悲)', factor: 'SORROW', tScore: 48, A: 48, group: 'normal', groupLabel: '정상 범위' },
                { name: '공포', subject: '공포(恐)', factor: 'FRIGHT', tScore: 72, A: 72, group: 'risk', groupLabel: '위험 범위' },
                { name: '놀람', subject: '놀람(驚)', factor: 'FEAR', tScore: 58, A: 58, group: 'normal', groupLabel: '정상 범위' },
              ]
            },
            {
              id: 'demo-2',
              ...
              overallTScore: 45.2,
              scores: [
                { name: '기쁨', subject: '기쁨(喜)', factor: 'JOY', tScore: 58, A: 58, group: 'normal', groupLabel: '정상 범위' },
                { name: '분노', subject: '분노(怒)', factor: 'ANGER', tScore: 48, A: 48, group: 'normal', groupLabel: '정상 범위' },
                { name: '생각', subject: '생각(思)', factor: 'THOUGHT', tScore: 42, A: 42, group: 'normal', groupLabel: '정상 범위' },
                { name: '우울', subject: '우울(憂)', factor: 'DEPRESSION', tScore: 52, A: 52, group: 'normal', groupLabel: '정상 범위' },
                { name: '슬픔', subject: '슬픔(悲)', factor: 'SORROW', tScore: 45, A: 45, group: 'normal', groupLabel: '정상 범위' },
                { name: '공포', subject: '공포(恐)', factor: 'FRIGHT', tScore: 40, A: 40, group: 'normal', groupLabel: '정상 범위' },
                { name: '놀람', subject: '놀람(驚)', factor: 'FEAR', tScore: 38, A: 38, group: 'normal', groupLabel: '정상 범위' },
              ]
            }
          ]
        }
  ...
  if (allResults.length === 0) {
    return (
      <div className="min-h-screen bg-[#fcfdfc] flex flex-col items-center justify-center p-6 text-center">
        ...
        <h2 className="text-2xl font-extrabold text-[#222] mb-2">검사 데이터가 없습니다</h2>
        <p className="text-gray-500 mb-8 max-w-sm">먼저 /questionnaire에서 감정 검사를 완료해주세요.</p>
        <button onClick={() => router.push('/questionnaire')} className="bg-[#566e63] text-white px-8 py-3.5 rounded-full font-bold shadow hover:bg-[#43574d] transition-colors">
          감정 검사하러 가기
        </button>
      </div>
    )
  ...
  const createdDate = new Date(selectedResult.timestamp || Date.now())
    ...
    <div className="min-h-screen bg-[#f3f6f4] font-sans text-[#333]">
        ...
        <div className="max-w-[900px] mx-auto">
          ...
          >
            {allResults.map((result, idx) => {
              ...
              const dateStr = resultDate.toLocaleDateString('ko-KR', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })
              const tScore = result.overallTScore || result.overall_t_score || 0
              return (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 mb-16 bg-gray-50 p-4 sm:p-6 rounded-2xl border border-gray-200">
             ...
             <div>
                ...
                <div className="flex items-center gap-2">
                   ...
                   <div className="font-extrabold text-sm text-[#222]">{totalTScore.toFixed(1)}</div>
                </div>
             </div>
             <div>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        @media print {
          ...
          [class*="border-b border-gray-200 sticky"] { display: none !important; }
