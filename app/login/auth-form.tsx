...
function SubmitButton({ isLogin }: { isLogin: boolean }) {
  ...
  return (
    ...
    >
      {pending ? 'ì²˜ë¦¬ ì¤?..' : (isLogin ? 'ë¡œê·¸?? : '?Œì›ê°€??)}
    </button>
  )
}
  ...
  return (
    <div className="w-full max-w-[400px] flex flex-col items-center">
      ...
      <div className="w-full space-y-3 mb-6">
        ...
        <div className="flex gap-3">
          <button type="button" className="flex-1 py-3 px-4 bg-[#03C75A] text-white rounded-full flex items-center justify-center gap-2 text-sm font-bold hover:bg-[#02b351] transition-colors">
            <span className="font-extrabold pb-[2px]">N</span> ?¤ì´ë²?          </button>
      ...
      <div className="w-full relative flex items-center justify-center mb-6">
        ...
        <span className="bg-[#faf8f5] px-4 text-xs text-gray-600 relative z-10">?ëŠ” ?´ë©”?¼ë¡œ ë¡œê·¸??/span>
      </div>
      ...
      <form action={isLogin ? loginAction : signupAction} className="w-full flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-2">
          <label className="text-sm text-gray-600 font-medium">?´ë©”??ì£¼ì†Œ</label>
          <input 
            name="email"
            type="email" 
        ...
        <div className="flex flex-col gap-2">
          <div className="flex justify-between items-center">
            <label className="text-sm text-gray-600 font-medium">ë¹„ë?ë²ˆí˜¸</label>
            {isLogin && (
              <a href="#" className="text-xs text-gray-500 font-medium">ë¹„ë?ë²ˆí˜¸ ì°¾ê¸°</a>
            )}
          </div>
      ...
      <div className="mt-8 text-sm text-gray-600 font-medium">
        {isLogin ? "ê³„ì •???†ìœ¼? ê??? " : "?´ë? ê³„ì •???ˆìœ¼? ê??? "}
        <button 
          onClick={() => setIsLogin(!isLogin)} 
          className="text-[#566e63] font-bold hover:underline"
        >
          {isLogin ? "?ˆë¡œ???ˆì‹ì²?ë§Œë“¤ê¸? : "ì§€ê¸?ë¡œê·¸?¸í•˜ê¸?}
        </button>
      </div>
    </div>
  )
